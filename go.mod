module github.com/adrienkohlbecker/m3u

go 1.16

require (
	github.com/deckarep/golang-set v0.0.0-20160220235750-1f0f4ff8d3fb
	github.com/fatih/color v1.0.0 // indirect
	github.com/go-errors/errors v1.4.1-0.20210806162005-67e195e4c6a4
	github.com/mattn/go-colorable v0.0.4 // indirect
	github.com/mattn/go-isatty v0.0.0-20160806122752-66b8e73f3f5c // indirect
	github.com/pkg/xattr v0.0.0-20160813101438-6c6c9802872b
	github.com/ushis/m3u v0.0.0-20150127162843-94396b784733
	golang.org/x/text v0.3.8
	gopkg.in/cheggaaa/pb.v1 v1.0.5
)
